  ...
  function initTournamentView(tour: Tournament) {
    ...
    if (tour.groups && tour.groups.length > 0) {
      tour.groups.forEach((g) => {
        const names = g.players.map((p) => p.displayName);
      ...
      <main className="px-4 py-4 space-y-4 max-w-md sm:max-w-lg mx-auto">
        ...
        <section className="bg-white rounded-2xl p-4 border border-slate-200/80 shadow-2xs space-y-2">
          <h3 className="text-xs font-bold text-slate-800 flex items-center justify-between">
            ...
            <span className="text-[10px] text-emerald-700 font-semibold">3개 조 자동 분배</span>
          </h3>
        ...
        {/* KDK Groups & Match Input */}
        <section className="space-y-4">
          <div className="flex items-center justify-between">
            <h2 className="text-sm font-extrabold text-slate-900 flex items-center gap-1.5">
              <Trophy className="h-4 w-4 text-emerald-600" /> KDK 복식 대진 및 성적표
            </h2>
          ...
          {Object.keys(groupMatches).map((groupName, gIdx) => {
            const matches = groupMatches[groupName];
            const playerSet = Array.from(
              new Set(matches.flatMap((m) => [...m.team1, ...m.team2]))
            );
            const standings = calculateGroupStandings(playerSet, matches);
            ...
            return (
              ...
              >
                ...
                <div className="bg-slate-900 text-white px-4 py-3 flex items-center justify-between">
                  ...
                  <span className="text-[10px] text-slate-300">4인 복식 3라운드</span>
                </div>
                    ...
                    <div className="overflow-x-auto rounded-xl border border-slate-200">
                      <table className="w-full text-[11px] text-left">
                        <thead className="bg-slate-100 text-slate-600 font-bold border-b border-slate-200">
                          <tr>
                            <th className="py-2 px-2.5 text-center">순위</th>
